...
def run_simulation_one(hw_name:str, 
                       ...
                       seq_type:str = "XpXm"):
    token = qiskit_wrapper.get_active_token(remaining=50, repetition=0, token_number=1)[0][0]
    print(token)
...
def run_simulation_all(hw_name:str, 
    ...
    print(hw_name, shots, type, threshold_avg, threshold_lcd, seq_type)
    ...
    if type == "n4_x":
        ...
        run_simulation_one(hw_name, noise_levels, file_path=file_path, 
                        compilations=["qiskit_3"], triq_measurement_type="polar_meas", 
                        repeat=4, shots=shots,
                        mp_options=mp_options, seq_type=seq_type  )
        
        # prune-lcd
        print("Start prune-lcd...")
        mp_options = {"enable":True, "execution_type":"final"}
...
for i in range(1):
    ...
    if not conf.use_ibm_cloud:
        try:
            run_simulation_all("ibm_kyiv", 4000, type_call, seq_type="XpXm")
            run_simulation_all("ibm_kyiv", 4000, type_call, seq_type="XY4")
            pass
        ...
        try:
            run_simulation_all("ibm_sherbrooke", 4000, type_call, (0.045,0.20), (0.045,0.20), seq_type="XpXm")
            run_simulation_all("ibm_sherbrooke", 4000, type_call, (0.045,0.20), (0.045,0.20), seq_type="XY4")
            pass

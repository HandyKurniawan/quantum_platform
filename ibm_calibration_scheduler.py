    ...
    try:
        ...
        for result in results:
            ...
            try:
                ...
                json_url = "https://quantum.cloud.ibm.com/api/v1/public/backends/"+ hw_name +"/properties"
                ...
                if response.status_code == 200:
                    ...
                    # Save the date for the general values
                    for g in json_data['general']:
                        pDate = str.replace(p["date"], "Z", "")
                        pName = g["name"]
                        pUnit = g["unit"]
                        pValue = g["value"]

                        cursor.execute('''
                            INSERT INTO ibm_general_spec (
                                calibration_id, date, name, unit, value
                            ) VALUES (%s, %s, %s, %s, %s)
                            ''', (
                                calibration_id, pDate, pName, pUnit, pValue
                            ))

                    # Save the date for the general qlists values
                    # Save the date for the general qlists values
                    for g in json_data['general_qlists']:
                        pName = g["name"]
                        pQubits = str(g["qubits"])

                        cursor.execute('''
                            INSERT INTO ibm_qlist_spec (
                                calibration_id, name, qubits
                            ) VALUES (%s, %s, %s)
                            ''', (
                                calibration_id, pName, pQubits
                            ))

                    # Commit the changes and close the connection
                    conn.commit()

                    # datetime.datetime.strptime(q[0]["date"], "%Y-%m-%dT%H:%M:%SZ"),
                    # datetime.datetime.strptime(q[1]["date"], "%Y-%m-%dT%H:%M:%SZ"),
                    # datetime.datetime.strptime(q[2]["date"], "%Y-%m-%dT%H:%M:%SZ"),
                    # datetime.datetime.strptime(q[3]["date"], "%Y-%m-%dT%H:%M:%SZ"),
                    # datetime.datetime.strptime(q[4]["date"], "%Y-%m-%dT%H:%M:%SZ"),
                    # datetime.datetime.strptime(q[5]["date"], "%Y-%m-%dT%H:%M:%SZ"),
                    # datetime.datetime.strptime(q[6]["date"], "%Y-%m-%dT%H:%M:%SZ"),
                    # datetime.datetime.strptime(q[7]["date"], "%Y-%m-%dT%H:%M:%SZ")
